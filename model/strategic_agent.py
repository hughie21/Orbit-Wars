"""
Strategic agent for Orbit Wars — WorldModel-based planning engine.

Port of the reference Hyperion-style agent featuring:
  - Timeline simulation per planet (arrival ledger, combat resolution)
  - Iterative convergent aiming with orbit prediction
  - Multi-source coordinated swarm / snipe / reinforce / recapture missions
  - Domination-aware adaptive margins
  - Soft real-time deadline management

Usage:
    python train.py --agent strategic --opponent heuristic --num_episodes 10
"""
...
class WorldModel:
    ...
    def projected_state(self, target_id, arrival_turn, planned_commitments=None, extra_arrivals=()):
        ...
        arrivals = [
            item for item in self.arrivals_by_planet.get(target_id, [])
            if item[0] <= cutoff
        ]
        arrivals.extend(
            item for item in planned_commitments.get(target_id, [])
            if item[0] <= cutoff
        )
    ...
    def min_ships_to_own_by(
        ...
        arrival_turn=None, planned_commitments=None, extra_arrivals=(), upper_bound=None,
    ):
        planned_commitments = planned_commitments or {}
    ...
    def ships_needed_to_capture(
        self, target_id, arrival_turn,
        planned_commitments=None, extra_arrivals=(),
    ):
...
def stacked_enemy_proactive_keep(planet, world):
    ...
    for enemy in world.enemy_planets:
        seeded = world.best_probe_aim(
            enemy.id, planet.id, max(1, int(enemy.ships)),
        )
        if seeded is None:
            continue
        _, aim = seeded
...
def settle_plan(
    src, target, src_cap, send_guess,
    world, planned_commitments, modes, policy,
    mission="capture", eval_turn_fn=None, anchor_turn=None,
    anchor_tolerance=None, max_iter=4,
):
    if src_cap < 1:
        return None
    ...
    candidate_sends = sorted(
        ...
        key=lambda send: (
            0 if mission != "snipe" or anchor_turn is None
...
def build_rescue_missions(world, policy, planned_commitments, modes):
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            missions.append(
                Mission(
                    kind="rescue", score=score, target_id=target.id,
                    turns=fall_turn, options=[option],
                )
            )
...
def build_recapture_missions(world, policy, planned_commitments, modes):
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            missions.append(
                Mission(
                    kind="recapture", score=score, target_id=target.id,
...
def build_reinforce_missions(world, policy, planned_commitments, modes, inventory_left_fn):
    ...
    for target in world.my_planets:
        ...
        for src in world.my_planets:
            ...
            missions.append(
                Mission(
                    kind="reinforce", score=score, target_id=target.id,
                    turns=fall_turn, options=[option],
                )
            )
...
def build_crash_exploit_missions(world, policy, planned_commitments, modes):
    ...
    for crash in detect_enemy_crashes(world):
        ...
        for src in world.my_planets:
            ...
            missions.append(
                Mission(
                    kind="crash_exploit", score=score, target_id=target.id,
                    turns=turns, options=[option],
                )
            )
    ...
    def compute_live_doomed():
        ...
        for planet in world.my_planets:
            status = world.hold_status(
                planet.id, planned_commitments=planned_commitments,
    ...
    for target_id, options in source_options_by_target.items():
        ...
        for i in range(len(top_options)):
            for j in range(i + 1, len(top_options)):
                ...
                missions.append(
                    Mission(
                        kind="swarm", score=pair_score, target_id=target_id,
            ...
            for i in range(len(top_options)):
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        ...
                        missions.append(
                            Mission(
                                kind="swarm", score=trio_score, target_id=target_id,
...
def build_world(obs):
    ...
    return WorldModel(
        ...
        initial_by_id=initial_by_id, ang_vel=ang_vel,
...
class StrategicAgent(BaseAgent):
    ...
    def act(self, observation):
        try:
            ...
            import sys
            import traceback
            print(f"StrategicAgent error: {traceback.format_exc()}", file=sys.stderr)
            return []
...
def strategic_agent(obs, config=None) -> list:
    ...
    if _agent_instance is None or _agent_instance.player_id != player:
        _agent_instance = StrategicAgent(player_id=player)
